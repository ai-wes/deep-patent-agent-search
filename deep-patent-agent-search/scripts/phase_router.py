...
def main() -> int:
    ...
    if args.auto_run or not args.headless:
        ...
        # Choose app name: env override, else list-apps (preferring domain_agents), else fallback to "app"
        def pick_app_name() -> str:
            explicit = os.environ.get("ADK_APP_NAME")
            if explicit:
                return explicit
            try:
                r = requests.get(f"{base_url}/list-apps", timeout=3)
                if r.ok:
                    names = r.json()
                    if isinstance(names, list):
                        if "domain_agents" in names:
        ...
        run_paths = [
            ...
            f"{base_url}/api/run",
            f"{base_url}/run_sse",
            f"{base_url}/api/run_sse",
        ]
        run_ok = False
        for run_url in run_paths:
            try:
                resp = requests.post(run_url, json=payload, timeout=180)
                if resp.ok:
                    print(f"[phase-router] /run completed via {run_url} for agent={agent}, phase={args.phase}")
                    run_ok = True
                    break
                else:
                    print(f"[phase-router] /run attempt {run_url} failed {resp.status_code}: {resp.text[:200]}")
            except Exception as exc:  # noqa: BLE001
                print(f"[phase-router] ERROR calling {run_url}: {exc}")
                continue
