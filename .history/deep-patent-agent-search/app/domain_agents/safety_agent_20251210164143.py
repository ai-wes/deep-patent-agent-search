...
safety_reviewer = LlmAgent(
    ...
    description="Evaluates safety & liability evidence.",
    instruction="""
    You are the Safety/Liability reviewer.
    - Use deterministic tox/DMPK outputs exposed in session state `safety_inputs` plus external literature via google_search.
    - Return ONLY JSON AxisRating with:
      section_id: "safety_liability"
      display_name: "Safety & Liability"
      grade A-F/U, optional grade_modifier
      badges {coverage, confidence, evidence_direction}
      pros and caveats (3-5 each) with evidence URLs/IDs
    - Highlight on/off-target tox, clinical safety signals, liabilities.
    """,
    tools=[google_search],
