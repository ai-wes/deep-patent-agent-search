...
risk_config_agent = LlmAgent(
    ...
    model=config.worker_model,
    description="Calibrates GB-TAR domain weights and RAG thresholds based on comprehensive risk research.",
    instruction="""
    You are the Risk Configurator.
    - Use comprehensive risk research from `risk_research` and strategic analysis from `risk_strategy` in session state.
    - Also analyze provided risk inventory and GB-TAR defaults in session state (risk_inventory, default_weights, default_rag).
    - Calibrate domain weights based on the relative importance and risk levels identified in the research.
    ...
    - Base calibration on the comprehensive risk research and strategic insights provided.
    """,
    tools=[google_search],
...
risk_triage_agent = LlmAgent(
    ...
    model=config.worker_model,
    description="Scores individual risks and assigns RAG/escalation based on comprehensive research and strategy.",
    instruction="""
    ...
    - Leverage the research data to provide context and benchmarking for risk scoring.
    - Return ONLY JSON array matching RiskTriageItemOutput (triage_items list).
    - Base your triage decisions on the comprehensive research and strategic insights provided.
    """,
    tools=[google_search],
...
ip_watch_agent = LlmAgent(
    name="ip_watch_agent",
    model=config.worker_model,
    description="Recommends top IP monitoring actions based on comprehensive research, signals, and FTO context.",
    instruction="""
    You are the IP Monitoring Advisor.
    - Use comprehensive IP/FTO research from `risk_research` and strategic analysis from `risk_strategy` in session state.
    - Also use patent signals, fto_brief, and triage context in session state.
    - Leverage the research data to identify critical patents, competitive IP positions, and monitoring priorities.
    - Return ONLY JSON with up to 5 watch_items (type, value, cadence, rationale with citation).
    - Base recommendations on the comprehensive research and strategic insights provided.
    """,
    tools=[google_search],
...
risk_gap_agent = LlmAgent(
    name="risk_gap_agent",
    model=config.worker_model,
    description="Surfaces emerging or missing risks based on comprehensive research and triage results.",
    instruction="""
    You are the Risk Gap Analyst.
    - Use comprehensive risk research from `risk_research`, strategic analysis from `risk_strategy`, and triage results from `risk_triage` in session state.
    - Identify newly emerging or missing risks given triage results and domain coverage.
    - Compare identified risks against the comprehensive research to find gaps.
    - Return ONLY JSON gap_items with category, risk, trigger, impact, mitigation, gate, status.
    - Base gap analysis on the comprehensive research and strategic insights provided.
    """,
    tools=[google_search],
...
reg_snapshot_agent = LlmAgent(
    ...
    model=config.critic_model,
    description="Synthesizes a markdown snapshot of regulatory/CMC posture based on comprehensive research and analysis.",
    instruction="""
    Write markdown starting with '# Regulatory & CMC Snapshot'.
    Use:
    - risk_research (comprehensive risk research data)
    - risk_strategy (strategic risk analysis)
    - risk_config, risk_triage, ip_watch (risk assessment outputs)
    - Provided target metadata in session state
    Sections to include: Regulatory Outlook, CMC & Supply, Escalation Guidance.
    Base analysis on all available research, strategy, and assessment outputs.
...
risk_pipeline = SequentialAgent(
    name="risk_pipeline",
    description="Researches risk data, performs strategic analysis, synthesizes brief, calibrates risk model, triages risks, recommends IP monitoring, identifies gaps, and drafts regulatory snapshot.",
    sub_agents=[
        risk_researcher,
        risk_strategist,
        risk_strategic_brief,
        risk_config_agent,
        risk_triage_agent,
        ip_watch_agent,
        risk_gap_agent,
